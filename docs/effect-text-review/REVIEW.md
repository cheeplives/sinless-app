...
### The sweep that was pointed at the wrong columns

Two rows came through the pass unchanged that should not have: `rig_mods` **Bonus
Link** (`+1 Link`, column `Link` = 1) and `heritage_features` **Rat** (`+1 to all
tests`, column `All` = 1). Both are live columns — `rigStats` adds `Link`
(rules.js:3345) and `all_skills_bonus` sums `All` (rules.js:1315) — so each row
stated a number twice.

They were missed for one reason: the sweep that found this pattern in `augments`
worked from a hand-written list of columns (the six attributes, the two armors)
and nobody thought to add `Link` or `All`. `column_echo.py` takes the column
names from the data itself, so a column added next year is covered without anyone
remembering to update it.

Pointing it at the *proposed* text finds **25 more**, none of them in `augments`
prose that the pass already rewrote:

| Where | Rows | |
|---|---|---|
| `heritage_features` | 14 | Dog, Cat, Bear, Snake, Octopus, Racoon, Chameleon, Camouflage, Chimerical, Redcap, Tough (×2), Arcano-Manon, Polypedal Legs 4/6/8 — all restating `Observation`, `Soak`, `Recon`, `Sorcery`, `Athletics`, `ZP` or the armor columns |
| `augments` | 6 | Strength Enhancement 1–6 — `+N Strength to all limbs`, where "to all limbs" is real content the grant sentence has to keep |
| `augments` | 1 | Hyperthyroid — the main sweep skipped it because the sentence is not *only* numbers ("Increase lifestyle cost 10%.") |
| `augments` | 2 | Cybertechtronic Ears and Eyes — **false positives**: "can mount up to 0.5 ZR of mods" is a capacity that happens to equal the row's own `ZR` cost. Two different quantities, same number. |

The heritage cluster is the substantive one and is **not** applied here: it is a
second rewrite of a table this pass has already been through, and it wants the
same review the augment cluster got rather than a silent sweep.

### CHECK (21) — all answered

| Table | Row | Note |
|---|---|---|
| `augments` | Delux Trackmobi | **TYPO** — "chasis" corrected to "chassis". **CHECK** — the row's Name ("Delux Trackmobi") looks like a misspelling of "Deluxe"; not changed here since renaming a row is out of scope (needs a `RENAMED_*` migration map per the style guide) — **RESOLVED (approved)**: rename to **Deluxe Trackmobi**. The owner confirms no saved character references it, so the usual orphaning risk does not apply — a `RENAMED_AUGMENTS` entry is still cheap insurance. The rename is a `Name`-column change and lands in `data.js` separately from this wording pass. |
| `augments` | Adrenal Pump | **UNCLEAR** — "Recharge: 30/min" in the original is ambiguous (a 30-minute recharge time, vs. some per-minute rate); rendered here as "30 minutes" to match how "Recharge:" reads elsewhere in this table, but the true intent should be checked against the source — **RESOLVED (approved)**: 30 minutes is correct — a 30-minute recharge, matching how "Recharge:" reads elsewhere in the table. |
| `heritage_features` | Nature Bound | **CHECK**: kept "UV" instead of spelling it to "ultraviolet" — the sense parser matches literal `ultraviolet` and would misread this vulnerability as granting Ultraviolet vision. Do not expand the abbreviation here. — **RESOLVED (approved)**: keep `UV`. Confirmed — expanding it would make `deriveSenseNotes` read a *vulnerability* as a grant of Ultraviolet vision. Needs a QA case before this document is deleted; it is the only place the reason is written down. |
| `martial_arts` | Way of the Tank | **CHECK**: kept the literal word "vs" — the style guide's normal rule would spell it out to "versus", but the dodge-dice veto only fires on literal `\bvs\b`/`\bif\b`. Writing "versus" here would silently turn this into a permanent +4d Dodge bonus. Do not canonicalize this word. — **RESOLVED (approved)**: keep the literal `vs`. Confirmed — writing "versus" removes the dodge-dice veto and silently converts this into a permanent +4d Dodge. Covered by **P15** (`docs/qa/passes/P15-effect-text-landmines.md`), so it survives this document. |
| `drones` | VSTOL Bird | **CHECK**: the -6d is a penalty on the drone's TARGET, not the operator. It currently produces no hit only because the text says "Stealth" and not the real skill name "Shadow" — pure luck, per the task brief. Deliberately kept "Stealth" (not "Shadow") in the same clause as the number: writing "Shadow" here would make `droneSkillDice` read it as a real -6d Shadow malus handed to the drone's OPERATOR. Do not rename this to "Shadow" without also restructuring how this row is parsed. — **RESOLVED (approved)**: the -6d applies when the **drone is the target**, confirming the reading here. Keep `Stealth` beside the number — writing the real skill name `Shadow` would make `droneSkillDice` hand a -6d Shadow malus to the drone's OPERATOR, inverting who is penalised. Covered by **P15** (`docs/qa/passes/P15-effect-text-landmines.md`), so it survives this document. Now that the semantics are confirmed the row could be restructured properly: a target-side penalty has no column to live in today. |
| `misc_gear` | Dorf | CHECK — not wired: "wound pen" is two characters short of `/wound penalt/i`, but fixing the wording alone can't make this work — `removesWoundPenalty` (rules.js:4987) is only ever applied to augments, martial-art levels, and heritage traits; `misc_gear` is never handed to it. Standardizing the wording is worth doing for consistency, but the immunity needs a rules.js change (teaching `removesWoundPenalty` to read `misc_gear.Effect`) before it can appear on a sheet. Dorf is the only drug in this table that claims wound-penalty immunity. — **RESOLVED — becomes an engine change (approved)**: a Dorf dose that has been *Used* and is still active should suppress wound penalties the way an augment does. Dorf already carries `Dose: 1`, so the missing half is `rules.js`: `removesWoundPenalty` (4987) is handed augments, martial-art levels and heritage traits, never `misc_gear`, and it would have to consider only LIVE doses rather than anything carried. **DONE** — `liveDoseRows` was added to `rules.js` and both wound-penalty scans now consult it, the Effect wording landed in `data.js` ahead of the rest of the pass (this row's Original was re-synced, which is why Original and Proposed now read alike), and **P06-029** covers it. Negation still beats doubling: a Dorf dose overrides a Reaction Enhancer. |
| `spells` | Shatter Ward | CHECK — original reuses the word "Force" for both the caster's Force and the ward's Force without distinguishing them (`Force<Force+Success`). Rewrite disambiguates per Description's wording; no number or comparison changed. — **RESOLVED (approved)**: rewording accepted as proposed. |
| `spells` | Forbidden Glamour of Accord | TYPO — "Negotiaion" → "Negotiation"; duplicated "vs vs" → "versus". CHECK — flat skill-dice bonus stated only in prose; style guide prefers a structured Skill Bonus column for this. — **RESOLVED (approved)**: accepted as proposed. The flat skill bonus stays in prose rather than moving to a `Skill Bonus` column. |
| `spells` | The Ancestral Working of the Savage Peal | CHECK — "Armor" added for clarity, sourced from Description (original just said "barriers<Force-1"). Also Effect's strict `<` is tighter than Description's "equal to or less than"; flagging the mismatch, not resolving it. — **RESOLVED (approved)**: accepted as proposed, strict `<` and all. |
| `spells` | The Infinite Illusion of Spiritual Seperation | CHECK — row name is misspelled ("Seperation"). Flagging only; not renaming, per the style guide (renames orphan saved characters). — **RESOLVED (approved)**: fix the spelling — **Separation**. A `Name`-column change, so it lands in `data.js` separately from this pass. Unlike Deluxe Trackmobi there is no `RENAMED_SPELLS` map in `rules.js` yet (only `RENAMED_AUGMENTS`, `RENAMED_SPIRITS`, `RENAMED_AMMO`), so one has to be added or a saved character holding this spell loses it. |
| `spells` | Confusion | CHECK — Effect specifies a d3, but Description rolls 1d6 and maps pairs (1-2/3-4/5-6) to the same three outcomes. Same odds, different die; confirm intended notation. — **QUERY — not applied.** The ruling given was "use the proposed with d2", but this row has **three** outcomes (attack ally / nothing / act normally) and the Proposed already reads `d3`, matching both the Effect and the Description's three paired 1d6 bands (1-2/3-4/5-6). A d2 cannot address three outcomes. Read here as the Horrors answer landing one row early, so `d3` stands — say so if a real change to two outcomes is intended. |
| `spells` | Despair | TYPO — stray "for in area" corrected to "in area". CHECK — Description adds a "(minimum 1)" floor on this penalty that Effect omits; without it, Force 1 would round to zero penalty dice. Flagging, not adding the number. — **RESOLVED (approved)**: accepted as proposed. The Description's "(minimum 1)" floor stays out of the Effect text. |
| `spells` | Enthrall | CHECK — Effect's brackets (`Int<15` days, `Int>15` hours) leave exactly Int=15 undefined; Description assigns Int=15 to the hours bucket ("fifteen and above"). Flagging the boundary gap, not resolving it. — **RESOLVED**: the owner edited the Proposed cell directly — Int 15 is assigned to the hours bucket, closing the boundary gap. |
| `spells` | Horrors of the Unknown Dark | CHECK — Effect specifies a d2, but Description rolls 1d6 and maps halves (1-3/4-6) to the same two outcomes. Same odds, different die; confirm intended notation (same pattern as Confusion, row 26). — **RESOLVED (approved)**: `d2` confirmed, which is what the Proposed already reads. Two outcomes, matching the Description's two 1d6 bands (1-3/4-6). |
| `spells` | Shadow Anchor | CHECK — lowercased "shadow" (the caster's own shadow) to avoid visual confusion with the capitalized **Shadow** skill used elsewhere in this data. Interpretive call, not pure formatting — flagging. — **RESOLVED (approved)**: the lowercased "shadow" stands. |
| `spells` | The Thrity Cursed Servants of Athozog | CHECK — canonicalized "Recon" → "Reconnaissance" (`SKILLS` alias). Flat/Force-scaled skill bonus stated only in prose; style guide prefers a structured Skill Bonus column. — **RESOLVED (approved)**: accepted as proposed, `Reconnaissance` included. The row name's own misspelling ("Thrity") was not raised and is untouched. |
| `spells` | Bound Servant | CHECK — "+2d to all tests" (familiar's own bonus) and "+2d Sorcery/Channeling" (caster's bonus) are flat dice bonuses stated only in prose; style guide prefers structured columns for this. Neither names a Pool, so no parser risk either way. — **RESOLVED (approved)**: accepted as proposed; the dice stay in prose. Separate and still open: whether a *known* spell should grant its columns at all, which `gearSkillEffects` currently does unconditionally — see "Needs a ruling". |
| `rituals` | Weather Protection | CHECK — Description frames the temperature shift as "up to" 10°C toward 21°C (capped, not unconditional); Effect reads as a flat 10°C shift. Flagging the ambiguity, not resolving it (not adding "up to" without confirmation). — **RESOLVED (approved)**: as written — a flat 10°C shift, no "up to". |
| `programs` | Vermin Call 6 | CHECK: damage tiers are uneven — ranks 1–2 deal 1, ranks 3–5 deal 2 (three ranks), rank 6 jumps to 3 after only one rank at the middle tier. Not changed; confirm rank 6 (and the 3-wide middle tier) is intentional rather than a missing rank 7 or a rank-5 typo. — **RESOLVED (approved)**: as written. The uneven damage tiers are intentional. |
| `programs` | Hacking 6 | Every one of the six original thresholds (3,5,7,9,11,13) checks out against `MCP ≤ 2×Rating+1` and against the Description's "at least half the deck's MCP, round down" rule — no break found, formula collapse is safe. — **RESOLVED (approved)**: as written. |
| `hack_actions` | Destroy Camera Network | CHECK: identical Alert/Op Heat cost to "Destroy Single Camera" above despite the larger scope (a whole network vs. one camera). Not changed; confirm this is intentional. — **RESOLVED (approved)**: as written. Matching the single-camera cost is intentional. |
...
| 12 | Rat | +1 to all tests | Grants a bonus to all tests. | — | **Column duplicate** — the row carries `All: 1`, and `rules.js:1315` sums that column into `all_skills_bonus`. The prose was a second copy of a live number. Missed by the main sweep, which knew about the attribute and armor columns and had never been pointed at `All`; `column_echo.py` now derives the column list from the data instead of a hand-kept list. |
...
| 4 | Bonus Link | +1 Link | Grants a bonus to Link. | — | **Column duplicate** — the row carries `Link: 1`, and `rigStats` adds it (`links += toInt(asNumber(modRow.Link))`, rules.js:3345). Same miss as `heritage_features` Rat: this table's `Link` and `Hardening` columns were never in the sweep's list. |
...
| 5 | Dorf | Inhaled powder or hypopatch. Ignore wound penalties for 2 hours. -2d Finesse Pool for 12 hours. | Inhaled powder or hypopatch. Ignore wound penalties for 2 hours. -2d Finesse Pool for 12 hours. | `pool Finesse-2` | CHECK — not wired: "wound pen" is two characters short of `/wound penalt/i`, but fixing the wording alone can't make this work — `removesWoundPenalty` (rules.js:4987) is only ever applied to augments, martial-art levels, and heritage traits; `misc_gear` is never handed to it. Standardizing the wording is worth doing for consistency, but the immunity needs a rules.js change (teaching `removesWoundPenalty` to read `misc_gear.Effect`) before it can appear on a sheet. Dorf is the only drug in this table that claims wound-penalty immunity. — **RESOLVED — becomes an engine change (approved)**: a Dorf dose that has been *Used* and is still active should suppress wound penalties the way an augment does. Dorf already carries `Dose: 1`, so the missing half is `rules.js`: `removesWoundPenalty` (4987) is handed augments, martial-art levels and heritage traits, never `misc_gear`, and it would have to consider only LIVE doses rather than anything carried. **DONE** — `liveDoseRows` was added to `rules.js` and both wound-penalty scans now consult it, the Effect wording landed in `data.js` ahead of the rest of the pass (this row's Original was re-synced, which is why Original and Proposed now read alike), and **P06-029** covers it. Negation still beats doubling: a Dorf dose overrides a Reaction Enhancer. |
...
| 22 | The Infinite Illusion of Spiritual Seperation | Stun tgt 1 round. Failed resistance separates mind from body for undetermined time | Stuns target for 1 round. On failed resistance, separates mind from body for an undetermined time. | — | CHECK — row name is misspelled ("Seperation"). Flagging only; not renaming, per the style guide (renames orphan saved characters). — **RESOLVED (approved)**: fix the spelling — **Separation**. A `Name`-column change, so it lands in `data.js` separately from this pass. Unlike Deluxe Trackmobi there is no `RENAMED_SPELLS` map in `rules.js` yet (only `RENAMED_AUGMENTS`, `RENAMED_SPIRITS`, `RENAMED_AMMO`), so one has to be added or a saved character holding this spell loses it. |
...
| 44 | Shadow Anchor | Anchor tgt to their Shadow on the ground. Cannot move for duration, but can take other actions. | Anchors target to their shadow on the ground. Cannot move for Duration, but can take other actions. | — | CHECK — lowercased "shadow" (the caster's own shadow) to avoid visual confusion with the capitalized **Shadow** skill used elsewhere in this data. Interpretive call, not pure formatting — flagging. — **RESOLVED (approved)**: the lowercased "shadow" stands. |
...
| 46 | Create Darkenbeast | Turn animal into darkenbeast. Gains 1/2 Force in B/I Armor, +2dmg, +3 to Brawn/Finesse/Resolve. Caster gains Exploit action to control beast. Simple action to direct. | Turns animal into darkenbeast. The animal Gains Force/2 Ballistic/Impact Armor. Add 2 to its melee damage. Add 3 to the Animal's Brawn/Finesse/Resolve Pools. Caster gains Exploit action to control beast. Simple action to direct. | `pool Brawn+3, Finesse+3, Resolve+3` | **NEW BEHAVIOUR** — removes a pool toggle that should never have existed. `derivePoolEffects` reads spell Effects (`rules.js:4213`) and has no concept of the summoned beast, so `+3 to Brawn/Finesse/Resolve` landed on the **caster**. The +3 belongs to the animal; the rewrite says so, and `POOL_DICE_RE` no longer matches. Confirmed by the repo owner. Same reasoning as the five spells in the summary below. |
...
| 53 | Bound Servant | Costs 2 ZP. Gain animal as familiar that gets +2d to all tests. Gain Exploit action to direct familiar. Gain +2d to Sorcery/Channeling. If it dies, suffer 2d6 stun and a new familiar appears at dawn. | Costs 2 ZP. Gain an animal familiar; it gets +2d to all tests. Caster gains an Exploit action to direct the familiar. Caster gains +2d Sorcery/Channeling. If the familiar dies, caster suffers 2d6 stun and a new familiar appears at dawn. | — | CHECK — "+2d to all tests" (familiar's own bonus) and "+2d Sorcery/Channeling" (caster's bonus) are flat dice bonuses stated only in prose; style guide prefers structured columns for this. Neither names a Pool, so no parser risk either way. — **RESOLVED (approved)**: accepted as proposed; the dice stay in prose. Separate and still open: whether a *known* spell should grant its columns at all, which `gearSkillEffects` currently does unconditionally — see "Needs a ruling". |
...
- **New parser hits introduced:** 0. No row becomes newly parseable by `POOL_DICE_RE` or any other engine regex. One hit is **removed**: Create Darkenbeast (row 46) was the only spell reaching `POOL_DICE_RE`, and it was giving the *caster* the +3 meant for the darkenbeast. Dropping it is the point of the rewrite, not a casualty of it — see that row's note.
- **NEW BEHAVIOUR flags (loud, per the task's hard requirement):** 6 — rows 32 (Geas), 45 (The Uncountable Tendrils of Ehon), 46 (Create Darkenbeast), 48 (Evocation of the Frail Beam of Debility), 59 (Summon Elemental), 62 (Natural Fury). Row 46 is the only one that *changes* what the engine does today; the other five are spells whose Description buffs/debuffs a Pool (the caster's or a summoned creature's), and all five were deliberately kept in a prose form that cannot trigger `POOL_DICE_RE` — either because the bonus is Force-scaled (no literal digit exists to sign) or because the affected stat belongs to a summoned creature, not the caster. Flagging these is a design question for the repo owner: should any of these ever become an actual toggle, and for whom?
...
| 78 | Vermin Call 6 | Target is swarmed by vermin: cannot take complex actions and takes 3 direct physical damage per round. Power armor, synthetics, spirits, and software agents are immune. | Swarms the target with vermin. The target cannot take complex actions and takes 3 direct physical damage per round. Power armor, synthetics, spirits, and software agents are immune. | — | CHECK: damage tiers are uneven — ranks 1–2 deal 1, ranks 3–5 deal 2 (three ranks), rank 6 jumps to 3 after only one rank at the middle tier. Not changed; confirm rank 6 (and the 3-wide middle tier) is intentional rather than a missing rank 7 or a rank-5 typo. — **RESOLVED (approved)**: as written. The uneven damage tiers are intentional. |
...
| 4 | Destroy Camera Network | Req Attack software. Raise Alert and Op Heat by 1 | Requires Attack software. Raises Alert and Op Heat by 1. | — | CHECK: identical Alert/Op Heat cost to "Destroy Single Camera" above despite the larger scope (a whole network vs. one camera). Not changed; confirm this is intentional. — **RESOLVED (approved)**: as written. Matching the single-camera cost is intentional. |
